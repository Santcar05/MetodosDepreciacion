...
public class EscenaSumaDigitosController {
	...
	public void verificarActivo(KeyEvent event) throws Exception{
		long vActivo = Long.parseLong(valorActivo.getText());
	...
	public void verificarResidual(KeyEvent event) throws Exception{
		long vResidual =Long.parseLong(valorResidual.getText());
	...
	public void GenerarTablaSumaDigitos(ActionEvent event) {
		...
		try {
			...
			this.mensajeValorActivo.setText("Digite un numero por favor");
			error = true;
		}
		try {
			...
			this.mensajeValorResidual.setText("Digite un numero por favor");
			error = true;
		}
		try {
			...
			this.mensajeVidaUtil.setText("Digite un numero por favor");
			error = true;
		}	
		//Entrar a la escena de la tabla de linea Recta
		if(!error) {
			try {
